...
from python_functions.Global import *


from python_functions.entity import *
from python_functions.user import *
...
def log_login_attempt(user_id, action, remarks):
    """
    Logs the login attempt in the log_files table.
 
    Parameters:
    - user_id: The ID of the user attempting to log in
    - action: 'LOGIN' or 'LOGOUT'
    - remarks: Any remarks about the attempt, e.g., 'User not found', 'Successful login'
    """
    ...
    try:
        ...
        log_query = """
        INSERT INTO log_files (date, time, actor_id, `table`, table_key, field, action, previous_value, modified_value, remarks)
        ...
        table_name = 'users'  # This is the table where the user is being logged
        table_key = str(user_id)  # Set table_key as user_id
        field_name = 'user_id'  # Set field to indicate the user_id is being used
        previous_value = ''  # Placeholder for previous value
        modified_value = ''  # Placeholder for modified value
 
        # Debugging output to check the values being inserted
        print(f"Inserting log entry: {date}, {time}, {user_id}, {table_name}, {table_key}, {field_name}, {action}, {previous_value}, {modified_value}, {remarks}")
 
        # Execute the insert statement
        cursor.execute(log_query, (date, time, user_id, table_name, table_key, field_name, action, previous_value, modified_value, remarks))
        conn.commit()
 
        # Check the number of affected rows
        if cursor.rowcount > 0:
            print("Log entry successfully inserted.")
        else:
            print("No rows were inserted. Check the SQL statement and data.")
 
    except mysql.connector.Error as e:
        print(f'An error occurred while logging the attempt: {str(e)}')
...
def login():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user:
                ...
                if bcrypt.checkpw(password.encode('utf-8'), user['password'].encode('utf-8')):
                    ...
                    success_message = remarks
                    log_login_attempt(user['user_id'], 'LOGIN', remarks)
        ...
        log_login_attempt(user_id, 'LOGIN', remarks)
 
    # If the request method is GET or if authentication fails, render the login page
    return render_template('Global/login.html', error_message=error_message, success_message=success_message)
