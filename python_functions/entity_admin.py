...
#  Query 1: Fetch data from final_rcms_data table filtered by entity_id
        ...
        # Query 3: Fetch data from regulation_master table with regulation_name
        ...
        # Merge with regulation_master (which now includes regulation_name as Regulation)
...
html.Div([
    ...
    html.Div([
        # Section 1: Images
        html.Div([
            html.Img(src="{{/assets/main_converted.png}}", className='small-image'),  # First image
    ...
    # End of Activity Container
                ...
                html.Div([  # Data Container
                    ...
                    # First row: Two charts side by side
                        html.Div([
                            dcc.Graph(id='status-distribution-donut', className='chart',style={'margin-left':'-10%','margin-top':'15px'}),
                            
                            dcc.Graph(id='factory-activities', className='chart',style={'margin-left':'-8%','margin-top':'-75px'}),
                            dcc.Graph(id='status-distribution-bar', className='chart',style={'margin-left':'-5%','margin-top':'-65px'}),
                            ...
                            html.Div([
                                    ...
                                    dash_table.DataTable(
                                        ...
                                        style_data_conditional=[
                                            {'if': {'row_index': 'odd'}, 
                                            'backgroundColor': '#acc9e0'},  # Light Blue for odd rows
                                            {'if': {'row_index': 'even'}, 
    ...
    [Input('factory-activities', 'clickData'),
     ...
     Input('selected-time-range', 'data'),
     Input('selected-mandatory-optional', 'data'),
    Input('url', 'search')]  # Add 'search' input from the URL
    )
    def update_dashboard(statusClickData, criticalityClickData, 
                        donutClickData, n_intervals, selected_filter, 
                        selected_time_range, selected_mandatory_filter, search):
        ...
        color_map = {
            ...
            'Yet to Start': '#9E9E9E',  # Grey
            'WIP': '#2196F3',  # Blue
            'WIP with Delay': '#F44336',  # Red
            'Completed with Delay': '#FF9800',  # Amber
            'Yet to Start with delay': '#5C63F2'  # Dark Purple
        }
        ...
        yet_to_start_donut = create_donut_chart(not_started_activities, total_activities, '#888888')
        ...
        line_chart = px.line(df_monthly_task_count, x='month', y='task_count', color='Entity',
                    labels={'task_count': 'Task Count', 'month': 'Month'},
                    title='Monthly Task Count for Each Entity')
        ...
        margin=dict(l=10, r=10, t=25, b=25),  # Adjust margins
        title_x=0.5 ,
    ...
    # Bar Chart: Regulation-wise Activities by criticality breakdown with Custom Colors for Criticality
        ...
        grouped_data = default_data.groupby(['Entity', 'criticality', 'calculated_status']).size().reset_index(name='Count')

        # Handling specific chart clicks using dash.callback_context
        ctx = dash.callback_context
        if ctx.triggered:
            triggered_id = ctx.triggered[0]['prop_id'].split('.')[0]

            # If "status-distribution-donut" was clicked
            if triggered_id == 'status-distribution-donut':
                ...
                return (status_grouped_data[['Category', 'calculated_status', 'Count']].to_dict('records'), 
            ...
            if triggered_id == 'factory-activities':
                ...
                # Filter the DataFrame based on the selected factory and status
                # filtered_df = df[
                #     (df['Entity'] == clicked_factory) & 
                #     (
                #         (df['start_date'].notnull()) & (df['end_date'].isnull()) & (df['due_on'] > df['start_date']) | 
                #         (df['start_date'].isnull()) & (df['end_date'].isnull()) & (df['due_on'].notnull())
                #     )
                # ]

                filtered_df = df[
                    (df['Category'] == clicked_factory) & 
                    (
                        (df['due_on'] > current_date) 
                    )
                ]
                ...
                return filtered_df[['Category', 'Task', 'due_on']].to_dict('records'), columns, \
                    total_activities_donut, completed_activities_donut, completed_delay_donut, wip_activities_donut,\
            wip_delay_donut , yet_to_start_donut,yet_to_start_delay_donut,\
            status_distribution_donut, \
            factory_activities, status_distribution_bar ,line_chart
        ...
        if triggered_id == 'status-distribution-bar':
        ...
        return grouped_data[['Entity', 'criticality', 'calculated_status', 'Count']].to_dict('records'), [
