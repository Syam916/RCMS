...
# Function to generate OTP (add your own logic for OTP generation)
def generate_otp():
    import random
...
import random
import time
# Function to check if a password is valid (implement your own password policies)
def is_valid_password(password):
    """ Validate password strength: must be 8 characters, have upper/lowercase, digits, and special chars. """
    if len(password) < 8:
        return False
...
def forgot_password_main():
    ...
    user_id = request.form.get('user_id')  # This gets 'user_id' from the form
    print(f"Received user_id: {user_id}")
    email=''

    
    # Retrieve current step and OTP verification status
    step = request.form.get('step', 'request_otp')  # Default step is requesting OTP
    otp_verified = session.get('otp_verified', False)  # Check if OTP has already been verified

    if step == 'request_otp':
        print('request otp entered')
        # Resend OTP or request OTP for the first time
        ...
        print('verification entered')
        # OTP verification step
        otp_input = request.form.get('otp')

        # Check if the OTP has expired (valid for 10 minutes)
        if time.time() - session['otp_time'] > 600:  # OTP is valid for 600 seconds (10 minutes)
        ...
        if otp_input == session.get('otp'):
            session['otp_verified'] = True  # OTP is now verified, allow password reset
        ...
        print('reset entered')
        # Skip OTP verification if already verified
        if not otp_verified:
        ...
        hashed_password = hashed_password = bcrypt.hashpw(new_password.encode('utf-8'), bcrypt.gensalt())
