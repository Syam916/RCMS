...
def get_due_on(regulation_id, activity_id):
    """Calculates the due date for a regulation activity and checks for holidays."""
    conn, cursor = connect_to_database()
    if conn is not None:
        cursor = conn.cursor()
        try:
            ...
            if result:
                ...
                if frequency and frequency > 0:
                    due_on = frequency_timeline + relativedelta(months=12 // frequency)
 
                # Adjust due_on if it falls on a holiday
                while True:
                    cursor.execute("SELECT COUNT(*) FROM holiday_master WHERE Holiday_Date = %s", (due_on,))
                    is_holiday = cursor.fetchone()[0]
                    if is_holiday:
                        due_on += relativedelta(days=1)  # Move to the next day if it's a holiday
                    else:
                        break
